...
/**
 * Styled to mirror dropdown-menu.tsx (Espresso) — the two menus must look
 * identical; only the trigger interaction differs. When updating styles here,
 * update dropdown-menu.tsx too, and vice versa.
 */
...
gap-2 rounded px-2 py-1.5 text-base text-ink-gray-7 data-[variant=destructive]:text-ink-red-8
data-[variant=destructive]:*:[svg]:text-ink-red-8! data-[variant=destructive]:focus:bg-surface-red-3 [&_svg:not([class*='text-'])]:text-ink-gray-6 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0
...
function ContextMenuItem({
  ...
  return (
    <ContextMenuPrimitive.Item
      ...
      className={cn(BASE_ITEM_STYLES, className)}
      {...props}
    />
  )
}
...
function ContextMenuShortcut({
  ...
  return (
    <span
      ...
      className={cn("text-ink-gray-5 ms-auto text-xs tabular-nums", className)}
      {...props}
    />
  )
}
